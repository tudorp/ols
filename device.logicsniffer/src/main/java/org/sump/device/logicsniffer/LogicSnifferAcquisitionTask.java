...
public class LogicSnifferAcquisitionTask implements SumpProtocolConstants, AcquisitionTask {
	...
	public AcquisitionResult call() throws IOException, InterruptedException {
		...
		if (channelCount <= 0) {
			throw new InternalError(
					"Internal error: did not obtain correct number of channels (" + channelCount + ")?!");
		}

		// not final because it would be nice to update this in case of a
		// interrupted/partial capture
		...
		createSampleProcessor(samples.length, samples, callback).process();
	 ...
	 *            the sample count;
	 * @param aSampleValues
	 *            the sample values; Detaches the currently attached port, if
	 *            one exists. This will close the serial port.
	 */
		...
		if (conn != null) {
			try {
				...
				if (this.outputStream != null) {
					// XXX it seems that after a RLE abort command, the OLS
					// device no
					// longer is able to process a full 5x reset command.
					// However, we're
					// also resetting the thing right after we've started an
					// acquisition,
	 ...
	 *             in case the device profile manager could not be
	 *             found/obtained.
	 */
	 ...
	 * @throws IOException
	 *             in case the device could not be found, or in case of any
	 *             other I/O problem.
	 */
	private void detectDevice() throws IOException {
		...
		do {
			...
			// reset the device first; to ensure it is in the proper initial
			// state...
			this.outputStream.writeCmdReset();
			...
			try {
				...
				// We're not able to finish our read; no further effort in
				// detecting the
	 ...
	 *            the number of enabled groups (denotes the number of bytes for
	 *            one sample);
	 * @param aSampleCount
	...
	private int[] readSamples(final int aEnabledGroupCount, int aSampleCount) throws IOException, InterruptedException {
		...
		try {
			while (!Thread.currentThread().isInterrupted() && (offset >= 0) && (offset < length)) {
				...
				if (read < 0) {
					// If the read fails, it might be because a stop was
					// requested, but in order to keep the already read
					// samples, we just brake, without throw new EOFException();
					LOG.log(Level.WARNING, "Acquisition ended early! Was there Stop request or a communication error?");
					break;
				} else {
					count -= read;
					offset += read;
			...
			LOG.log(Level.WARNING, "Acquisition ended early with a IOException!");
			// The stream might generate an exception at this point if the
			// capture was interrupted,
			// but we continue without showing the exception
		} finally {
			try {
				...
				// The stream might be already closed at this point if the
				// capture was interrupted,
				// but we continue without showing the exception
				LOG.log(Level.WARNING,
						"Reseting the device failed due to communication issues. Is the sream still open?");
			}
		...
		// In case acquisition was stopped/interrupted, only complete capture
		// samples need to be considered
		int completeSamplesCount = offset / aEnabledGroupCount;
		int[] samples = new int[completeSamplesCount];

		// Normalize the raw data into the sample data, as expected...
		for (int i = (completeSamplesCount * aEnabledGroupCount - 1), j = 0; i >= 0; i--) {
