...
public class SignalUI extends ComponentUI {
	...
	/**
	 * The number of samples that are shown in a single view upon which is
	 * decided to draw the group summary and scope a bit sloppy.
	 */
	...
	private static RenderingHints createSignalRenderingHints(final boolean aUseAA) {
		...
		hints.put(RenderingHints.KEY_ANTIALIASING,
				aUseAA ? RenderingHints.VALUE_ANTIALIAS_ON : RenderingHints.VALUE_ANTIALIAS_OFF);
		hints.put(RenderingHints.KEY_ALPHA_INTERPOLATION, RenderingHints.VALUE_ALPHA_INTERPOLATION_SPEED);
		hints.put(RenderingHints.KEY_COLOR_RENDERING, RenderingHints.VALUE_COLOR_RENDER_SPEED);
		hints.put(RenderingHints.KEY_RENDERING, RenderingHints.VALUE_RENDER_SPEED);
		return hints;
			...
			if (element instanceof SignalElement) {
				...
				if (signalElement.isDigitalSignal()) {
					...
					if (signalElement.isEnabled()) {
						...
						for (DataAnnotation<?> ann : helper.getAnnotations(DataAnnotation.class, startTimestamp,
								endTimestamp)) {
	...
	private void paintCursors(final Graphics2D aCanvas, final SignalViewModel aModel) {
		...
		for (int i = 0; i < Ols.MAX_CURSORS; i++) {
			...
			if ((cursorXpos < 0) || !clip.contains(cursorXpos, clip.y)) {
				// Trivial reject: don't paint undefined cursors, or cursors
				// outside the
				// clip boundaries...
				continue;
			}
	 ...
	 *            the measurement information, cannot be <code>null</code> or
	 *            empty.
	 */
	...
	/**
	 * Transforms a raw captured value by eliminating the bits corresponding to
	 * the zero bits in the (group) mask by shifting to the right the desired
	 * bits over undesired bits
	 *
	 * @param rawCapturedValue
	 *            the full bits of the captured sample
	 * @param groupMask
	 *            the mask associated to the group
	 * @return value associated to the bits selected in the mask in their
	 *         original magnitude order (please not that the bit order is not
	 *         changed accordingly to the signal order selected in the UI group,
	 *         but the bits are only grouped together to the right)
	 */
	private int transformGroupValueUsingGroupMask(int rawCapturedValue, int groupMask) {
		...
		while (groupMask != 0) {
			...
			groupMask <<= leadingZeros;
			rawCapturedValue <<= leadingZeros;
			// shifting left the transformed value to make space for the next
			// bit
			transformedValue <<= 1;
	...
	private void paintSignals(final Graphics2D aCanvas, final SignalViewModel aModel, final IUIElement[] aElements) {
		...
		for (IUIElement element : aElements) {
			...
			if (signalElement.isGroupSummary()) {
				...
				int textYpos = (int) ((signalElement.getHeight() + fm.getLeading() + fm.getMaxAscent()) / 2.0)
						- padding;
			...
			if (signalElement.isAnalogSignal()) {
				...
				// The max value is given by the number of one bits in the group
				// mask because the value will be trimmed of trailing zeros
				// and the inner bits corresponding to the zero bits in the mask
				// are extracted to form a continuous bit field
				int mask = signalElement.getMask();
				...
				if (startIdx == endIdx) {
					...
					for (int sampleIdx = startIdx; (p < POINT_COUNT) && (sampleIdx < endIdx); sampleIdx += sampleIncr) {
						...
						// Drawing a straight line from the previous different
						// timestamp value point
						// to the immediate previous timestamp of the current
						// timestamp;
						// this fixes the long, unreal, slopes that show up when
						// RLE compression is used
						// by creating the slope only between two real sampling
						// timestamps
						x[p] = (int) (zoomFactor * (timestamp - 1));
						y[p] = (int) (scaleFactor * previousSampleValue);
						p++;

						// Making sure we don't step over the allocated memory
						// "edge"
						if (p >= POINT_COUNT) {
							break;
						}
